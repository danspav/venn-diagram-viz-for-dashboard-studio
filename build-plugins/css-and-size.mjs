...
const IMPORT_REGEX = /@import\s+(?:"([^"]+)"|'([^']+)'|url\s*\(\s*["']?([^"')]+)["']?\s*\))\s*;/g;
    ...
    return {
        ...
        setup(build) {
            build.onLoad({ filter: /\.css$/ }, async (args) => {
                ...
                const contents = [
                    `(function(){ var s = document.createElement('style'); s.setAttribute('data-viz',''); s.textContent = ${escaped}; document.head.appendChild(s); })();`,
