...
const buildOptions = {
    ...
    loader: {
        // Images must stay inlined as data URLs, not shipped as separate static
        // files: Dashboard Studio renders custom visualizations in a sandboxed
        // iframe with origin "null", and Splunk's static file server sends no
        // Access-Control-Allow-Origin header for appserver/static/ assets. WebGL
        // refuses to upload a cross-origin, non-CORS-approved image as a texture
        // (texImage2D throws immediately), so any separately-fetched globe
        // texture is a dead end here — only inlined data: URLs work.
        '.png': 'dataurl',
        '.jpg': 'dataurl',
        '.jpeg': 'dataurl',
        '.gif': 'dataurl',
